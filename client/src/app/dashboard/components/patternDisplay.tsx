...
export default function PatternDisplay() {
  ...
  const handleSubmit = async (formData) => {
    ...
    try {
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/pattern/create`,
        formData,
        {
          withCredentials: true,
          headers: {
  ...
  return (
    <section className="flex max-w-6xl mx-auto p-4">
      <div
        className={`flex-1 w-full transition-width duration-300 ease-in-out ${selectedPatternId ? "pointer-events-none opacity-50" : ""}`}
      >
