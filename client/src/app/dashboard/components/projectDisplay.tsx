    ...
    const fetchProjects = async () => {
      try {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/project/all`,
  ...
  const handleSubmit = async (formData: FormData) => {
    ...
    try {
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/project/create`,
        formData,
        {
          withCredentials: true,
          headers: {
...
const ProjectImage = ({ patternId }: ProjectImageProps) => {
  ...
  useEffect(() => {
    const fetchPatternImage = async () => {
      try {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/pattern/${patternId}`,
          { withCredentials: true }
        );
