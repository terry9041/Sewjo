...
export default function FabricDisplay() {
    ...
    const handleSubmit = async (formData) => {
        ...
        try {
            ...
            console.error('Error creating fabric:', error.response ? error.response.data : error.message);
        }
    };
                    ...
                    {filteredFabrics
                        ...
                        >
                            ...
                            <div className="p-4 bg-background">
                                ...
                                <p className="text-sm text-muted-foreground">L: {fabric.length} {fabric.lengthInMeters ? "m" : "yd(s)"}</p>
                                <p className="text-sm text-muted-foreground">W: {fabric.width} {fabric.widthInCentimeters ? "cm" : "in"}</p>
                            </div>
                        </div>
                    ))}
                </div>
